...
const deleteRating = (req, res) => {
    const Book_booknum = req.params.Book_booknum;
    const Customer_custID = req.params.Customer_custID;
    pool.query(queries.deleteRating, [Book_booknum, Customer_custID], (error, results)=> {
        if (error) {
            // Handle the error gracefully
            console.error('Error deleting rating:', error);
            return res.status(500).send("An error occurred while deleting the rating.");
        }
